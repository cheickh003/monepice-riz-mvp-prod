...
export function registerCinetPayRoutes(app) {
...
async function initPayment(req, res) {
    try {
        ...
        const { orderId, amount, currency = 'XOF', customer, description, channels } = req.body;
        if (!orderId || !amount) {
...
async function ipnHandler(req, res) {
    try {
        ...
        const { cpm_trans_id: transactionId, cpm_amount: amount, cpm_currency: currency, cpm_result: result, cpm_trans_status: status, cpm_custom: custom, signature } = req.body;
        if (!transactionId) {
