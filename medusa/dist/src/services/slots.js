export class DeliverySlotService {
    constructor() {
        ...
        this.DEFAULT_SLOT_DURATION = 2; // hours
        this.DEFAULT_START_HOUR = 8; // 8 AM
        this.DEFAULT_END_HOUR = 20; // 8 PM
        this.DEFAULT_CUTOFF_HOURS = 3; // 3 hours before delivery
    }
    /**
     * Generate available delivery slots for a given date
     */
    generateSlots(options) {
        const { date, includeExpress = true, slotDurationHours = this.DEFAULT_SLOT_DURATION, startHour = this.DEFAULT_START_HOUR, endHour = this.DEFAULT_END_HOUR, cutoffHours = this.DEFAULT_CUTOFF_HOURS } = options;
        const slots = [];
     ...
     */
    generateExpressSlots(date, currentTime) {
        const slots = [];
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (slotStartHour >= 20)
                break; // Don't create slots after 8 PM
     ...
     */
    getSlotsForDateRange(startDate, endDate) {
        const result = {};
