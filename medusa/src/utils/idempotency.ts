...
export class IdempotencyService {
   ...
   */
  async isNewOperation(key: string, ttlSeconds: number = 86400): Promise<boolean> {
    try {
   ...
   * @param ttlSeconds - Time to live in seconds (default: 24 hours)
   */
  async markAsProcessed(key: string, ttlSeconds: number = 86400): Promise<void> {
    try {
   ...
   */
  static getIPNKey(transactionId: string): string {
   ...
   */
  static getPaymentInitKey(orderId: string): string {
